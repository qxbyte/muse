...
- **`src/app.tsx`** — Ink 根组件。`llm` / `settings` / `modelsRegistry` 是 mutable state——`/model` / `/config reload` 通过 setter 触发 **Agent 重建**；`messagesRef` 跨重建保留 messages。permission prompt / model selector / session selector / question picker 都是 modal overlay。
